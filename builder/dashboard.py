...
function renderGoogleRank() {{
  ...
  // ── 메인 차트 (이케아 제외) ──────────────────────────
  const mainDom = document.getElementById('chart-google-rank');
  if (mainDom && mainEntries.length) {{
    ...
    const mainChart = echarts.getInstanceByDom(mainDom) || echarts.init(mainDom);
  ...
  // ── 이케아 별도 패널 ─────────────────────────────────
  const ikeaDom = document.getElementById('chart-google-rank-ikea');
  if (!ikeaDom || !IKEA_KEY || !ikeaVal) return;

  ikeaDom.style.height = '100px';
  const ikeaChart = echarts.getInstanceByDom(ikeaDom) || echarts.init(ikeaDom);
