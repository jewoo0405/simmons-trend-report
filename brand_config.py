...
TREND_BATCHES = [
    ...
    ["시몬스", "씰리침대", "코웨이 비렉스"],                   # E: 씰리+코웨이 (시몬스 브리지로 교체 — 씰리침대 신호 약해 직접 앵커 사용)
    ["시몬스", "템퍼"],                                         # F: 템퍼 (시몬스 직접 앵커 — 체인 끊김 방지)
]
TREND_BRIDGES = ["시몬스", "시몬스", "에몬스", "씰리침대", "시몬스"]  # A→B, B→C, C→D, D→E, E→F 브리지 (마지막: 시몬스)

# 배치 비고 (max/min 20x 달성 불가 배치 주석)
TREND_BATCH_NOTES = {
    "E": "시몬스 직접 앵커로 교체 — 씰리침대 브리지(값=1.5) 신호 약함 해결.",
    "F": "템퍼 시몬스 직접 앵커 — 씰리침대 브리지 체인 끊김 방지.",
}
